...
public class UnweightedGraph<V> implements Graph<V> {
  ...
  public List<Integer> getACycle() {
	   List<Integer> list = new ArrayList<>();//create a list with all the vertices
	   for(int i = 0; i < vertices.size(); i++) {
		   list.add(i);
	   }
	    boolean[] isVisited = new boolean[vertices.size()];//create a boolean array to determine if a vertex has been visited (false by default)
	    int[] parent = new int[vertices.size()];//create an int array to determine parent of vertex (-1 by default)
	    for (int i = 0; i < parent.length; i++) {
			parent[i] = -1;
	    }	
	    while (list.size() > 0) { // check list size for this 
	        int v = list.get(0); //pick the first one, say v; 
	        Stack<Integer> stack = new Stack<>(); //create stack;
	        stack.push(v); //push v to the stack; 
	        isVisited[v] = true; //mark v visited; 
	        list.remove(new Integer(v)); //remove v from list with all vertices;
	        while (!stack.isEmpty()) { 
	            ...
	            if (neighbors.get(x).size() == 0) {
	            	...
	            	//for(int i = 0; i< neighbors.get(x).size(); i++) {
						...
						if(isVisited[e.v] == false) {
							...
							stack.push(e.v);
							isVisited[e.v] = true; 
	                      list.remove(new Integer(e.v)); //remove e.v from list with all vertices;
	                       neighbors.get(x).remove(i); //remove loop variable i from x's neighbor list;
	                       break;
	                   } 
	                   else if (e.v != parent[x]) {
	                       List<Integer> iList = new ArrayList<>(); //create an Integer ArrayList;
	                       iList.add(e.v);
	                       while (x != e.v && x != -1) {
	                           iList.add(x);
	                          x = parent[x]; //x is parent of x;
	                       }
	                       return iList;
	                   }
	                   else { 
