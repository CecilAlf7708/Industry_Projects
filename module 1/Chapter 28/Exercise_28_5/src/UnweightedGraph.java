...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public List<Integer> getPath(int u, int v) {
		...
		int curr = v;
		while (tree.getParent(curr) != -1) {
				list.add(curr); // add current to 0 spot of list
				curr = tree.getParent(curr); // current now equals the parent of current
		}
		
		if (curr == u) {
			list.add(curr); // add current to 0 spot of list
			return list; // we reached u from v, return our finished list
		}
		else {
                return null;  // path not found, return null here
		}
	}
